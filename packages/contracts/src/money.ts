 ...
 */
export const MoneyMinorSchema = z.string().regex(/^\d+$/, "amountMinor must be a non-negative integer string");

export const PositiveMoneyMinorSchema = z
  .string()
  .regex(/^[1-9]\d*$/, "amountMinor must be a positive integer string");

export const CurrencyCodeSchema = z.string().regex(/^[A-Z]{3}$/, "currency must use ISO-4217 style code");
